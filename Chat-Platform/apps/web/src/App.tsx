  ...
  if (!isAuthenticated && showAuth) {
    return (
      <AuthPage 
        ...
        onSuccess={() => { setShowAuth(false); setViewOverride(false); }} 
      />
    );
  }
      ...
      <div className="min-h-screen w-screen text-gray-200 antialiased flex flex-col relative overflow-x-hidden bg-[#000000] select-none">
        ...
        <div className="relative w-full pt-40 pb-32 flex flex-col items-center justify-center min-h-[85vh]">
          ...
          <main className="relative z-10 max-w-6xl mx-auto text-center px-6 flex flex-col items-center w-full">
            ...
            >
              <button 
                onClick={() => { if (isAuthenticated) setViewOverride(false); else setShowAuth(true); }}
        ...
        <div className="flex-1 overflow-y-auto px-2 py-3">
          <div className="mb-4">
            <div className="flex items-center justify-between px-2 mb-1 group/header relative">
              ...
              <button onClick={() => setShowCreateChannel(true)} className="text-[#949ba4] hover:text-[#dbdee1] transition-colors p-0.5">
                <Plus size={14} />
              </button>
            </div>
      ...
      <div className="flex flex-1 flex-col bg-[#313338] min-w-0">
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-baseline gap-2">
                    ...
                    {!msg.deleted && !isBeingEdited && (
                      <span className="opacity-0 group-hover:opacity-100 transition flex items-center gap-2 ml-1">
                        ...
                        {isOwnMessage && (
                          <>
                            <button onClick={() => handleStartEdit(msg)} className="text-[10px] text-gray-500 hover:text-amber-400 font-semibold">Edit</button>
                            <button onClick={() => handleRequestDelete(msg)} className="text-[10px] text-gray-500 hover:text-rose-400 font-semibold">Delete</button>
                          </>
                        )}
                      </span>
                    )}
                      ...
                      <div className="absolute z-30 mt-1 flex flex-col gap-1.5">
                        <div className="flex items-center gap-1 rounded-lg border border-gray-800 bg-[#1f2226] px-2 py-1.5 shadow-xl">
                          {QUICK_EMOJIS.map((emoji) => (
                            <button key={emoji} onClick={() => toggleReaction(msg, emoji)} className="text-base hover:scale-125 transition-transform px-0.5">{emoji}</button>
                          ))}
                          <div className="w-px h-5 bg-gray-800 mx-0.5" />
                          <button onClick={(e) => { e.stopPropagation(); setFullPickerFor(fullPickerFor === msg.id ? null : msg.id); }} className="text-gray-500 hover:text-gray-300 px-1" title="More emojis">
                            <Smile size={16} />
                          </button>
                        </div>
                        {fullPickerFor === msg.id && (
                          <div onClick={(e) => e.stopPropagation()}>
                            <EmojiPicker theme={Theme.DARK} onEmojiClick={(emojiData: EmojiClickData) => { toggleReaction(msg, emojiData.emoji); setFullPickerFor(null); }} width={300} height={350} />
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {msg.deleted ? (
                    <p className="text-sm mt-0.5 max-w-2xl leading-relaxed text-gray-600 italic">This message was deleted</p>
                  ) : isBeingEdited ? (
                    <div className="mt-1 flex items-center gap-2 max-w-2xl">
                      <input
                        ...
                        onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleSaveEdit(); } if (e.key === 'Escape') handleCancelEdit(); }}
                        autoFocus
                      ...
                      {msg.reactions.map((r) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs border transition ${reactedByMe ? 'bg-indigo-600/20 border-indigo-500/40 text-indigo-300' : 'bg-[#22252a] border-gray-800 text-gray-400 hover:border-gray-700'}`}
                          >
                            <span>{r.emoji}</span>
        ...
        <div className="p-4 bg-[#1a1d21]">
          <div className="relative flex items-end rounded-xl border border-gray-800 bg-[#22252a] px-4 py-3 focus-within:border-gray-700 shadow-lg transition">
            ...
            />
            <button onClick={() => setShowInputEmojiPicker(!showInputEmojiPicker)} className="ml-2 text-gray-500 hover:text-yellow-400 transition flex-shrink-0" title="Add emoji">
              <Smile size={18} />
            </button>
            {showInputEmojiPicker && (
              <>
                ...
                <div className="absolute bottom-12 right-2 z-30" onClick={(e) => e.stopPropagation()}>
                  <EmojiPicker theme={Theme.DARK} onEmojiClick={(emojiData: EmojiClickData) => { setMessageInput((prev) => prev + emojiData.emoji); }} width={300} height={350} />
                </div>
              </>
            )}
          </div>
